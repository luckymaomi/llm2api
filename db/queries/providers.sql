...
-- name: UpsertDiscoveredModel :one
...
UPDATE provider_credentials
SET status = 'retired', health_status = 'repair_required', encrypted_secret = sqlc.arg(encrypted_tombstone), cooldown_until = NULL,
...
VALUES (sqlc.arg(credential_id), sqlc.arg(model_id));
...
WHERE id = sqlc.arg(id) AND status = 'active'
  AND health_generation = sqlc.arg(health_generation);

-- name: RecordCredentialRuntimeFailure :exec
UPDATE provider_credentials
SET health_status = sqlc.arg(health_status), cooldown_until = sqlc.narg(cooldown_until),
    consecutive_failures = consecutive_failures + 1, last_error_kind = sqlc.arg(error_kind)
WHERE id = sqlc.arg(id) AND status = 'active'
  AND health_generation = sqlc.arg(health_generation);

-- name: AcquireCredentialHealthPermit :one
UPDATE provider_credentials
...
SELECT model.id, model.public_name, model.upstream_name, model.capabilities, model.created_at,
       ...
       EXISTS (SELECT 1 FROM gateway_key_models key_model
               WHERE key_model.gateway_key_id = sqlc.arg(gateway_key_id) AND key_model.model_id = model.id) AS key_authorized
FROM models model JOIN providers provider ON provider.id = model.provider_id
WHERE model.public_name = sqlc.arg(public_name);
