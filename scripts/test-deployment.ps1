param(
  [string] $ReleaseImage = "llm2api:deployment-release",
  [string] $CandidateImage = "llm2api:deployment-candidate",
  [switch] $SkipBuild
)
...
if ($project -notmatch '^llm2api-deployment-[a-z0-9-]+$') { throw "Generated deployment project name is unsafe." }
...
function Assert-HTTPSReady {
  ...
  do {
    ...
    $ErrorActionPreference = "Continue"
    try {
      $null = & curl.exe --silent --fail --insecure --output NUL "$env:LLM2API_DEPLOYMENT_URL/health/ready" 2>$null
...
function Assert-PublicMetricsBlocked {
  $status = & curl.exe --silent --insecure --output NUL --write-out "%{http_code}" "$env:LLM2API_DEPLOYMENT_URL/metrics"
...
function Set-DatabaseTarget {
  ...
  $dsn = "postgres://llm2api:$databasePassword@postgres:5432/$DatabaseName`?sslmode=disable"
  [IO.File]::WriteAllText($env:LLM2API_DATABASE_URL_FILE, $dsn, [Text.UTF8Encoding]::new($false))
}
...
try {
  ...
  $env:LLM2API_DATABASE_URL_FILE = Write-SecretFile "database-url" ""
  Set-DatabaseTarget -DatabaseName "llm2api"
  $env:LLM2API_VALKEY_PASSWORD_FILE = Write-SecretFile "valkey-password" $valkeyPassword
  $env:LLM2API_VALKEY_ACL_FILE = Write-SecretFile "valkey-acl" "user default on >$valkeyPassword ~* &* +@all"
  ...
  $env:LLM2API_VALKEY_IMAGE = "valkey/valkey:9.1.0-alpine"
  $env:LLM2API_CADDY_IMAGE = "caddy:2.10.2-alpine"
  $env:LLM2API_SITE_ADDRESS = "https://localhost"
  $env:LLM2API_ACME_EMAIL = "staging@example.invalid"
  ...
  & powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\backup-postgres.ps1 `
    -OutputPath $backupPath -Container $postgresContainer -DatabaseName llm2api `
    -DatabaseUser llm2api -ExpectedComposeProject $project
  ...
  $env:LLM2API_GATEWAY_IMAGE = $CandidateImage
  $env:LLM2API_ACTIVE_MASTER_KEY_VERSION = "99"
  ...
  & powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\restore-postgres.ps1 `
    -InputPath $backupPath -TargetDatabase llm2api_restored -Container $postgresContainer `
    -DatabaseUser llm2api -ExpectedComposeProject $project -ConfirmRestore
  if ($LASTEXITCODE -ne 0) { throw "PostgreSQL restore into the switch target failed." }
  Set-DatabaseTarget -DatabaseName "llm2api_restored"
  foreach ($service in @("gateway-a", "gateway-b")) {
    $null = Invoke-Compose -Arguments @("up", "--detach", "--no-deps", "--force-recreate", "--wait", $service)
    Assert-HTTPSReady
  }
  ...
  if ($LASTEXITCODE -ne 0 -or $backendMetrics -notmatch 'llm2api_admission_requests_total' -or $backendMetrics -notmatch 'llm2api_provider_attempts_total' -or $backendMetrics -notmatch 'llm2api_usage_operations_total') {
