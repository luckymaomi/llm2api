...
面向约 200～300 名受控用户的多 Provider LLM 网关，提供统一 API、成员管理、有效期授权、可靠路由和可审计用量。
...
- 上游 API Key 只由管理员集中保管；成员拿到的是路由和有效期受控的 API 密钥。
- 管理员从代码内置 Provider 能力创建资源池，维护上游 API Key、优先级、权重和模型路由；Provider 本身没有独立安装或编辑页面。
- 管理员直接创建和停用成员，发布路由套餐版本、分配有开始/到期时间的订阅和 API 密钥；成员只看自己的订阅、密钥与用量。
- 请求取消、上游 429/5xx、流中断、进程强杀和短暂基础设施故障都有明确恢复边界。
- PostgreSQL 保存权威事实，Valkey 负责可过期协调，并提供日志、指标、审计、备份和恢复工具。

LLMGateway 本身不处理采购金额、价格换算、充值、收款、开票或在线售卖套餐。公司先通过合同或内部审批确定服务，再由管理员在网关中分配访问路由和有效期。
...
4. 在“上游 API Key”点击唯一的“添加上游 API Key”，选择资源池后逐行粘贴凭据。每行可写 `名称,上游 API Key`，例如 `主 Key,sk-xxxxxxxx`；只粘贴 `sk-xxxxxxxx` 也可以。一行添加一个，多行一次添加多个，提交后逐项显示结果。添加完成后可编辑每条 Key 的模型和上游 RPM、TPM、并发容量，再执行单把测试或“测试全部 Key”；合法写入直接进入实时资格读取，不需要配置发布。
5. 在“套餐”创建并发布不可变版本，只选择这个套餐授权的模型/资源池路由。
6. 在“成员”直接创建成员并保存只显示一次的初始密码，再到“订阅”把已发布套餐版本分配给该成员。可设置开始和到期时间；到期留空表示永久有效。管理员本人无需订阅即可使用活动路由。
7. 进入“API 密钥”创建调用密钥，选择所属账号和精确的模型/资源池路由。完整密钥只显示一次；结果页会给出 Base URL、Python、Node.js 和 cURL 调用示例。
8. 在同一密钥行执行真实统一 API 测试；成功后再接入自己的 SDK。日常排障直接进入“运行状态”和“API 日志”，查看请求状态、输入/输出 Token、延迟和上游尝试。
...
停止命令会精确结束本仓库启动的 Gateway、管理网页和启动器，再停止 PostgreSQL/Valkey 容器。即使旧启动窗口已经关闭，它也能清理遗留进程；数据库卷会保留，下次启动会接着使用原成员、资源池、套餐、订阅、Key 和请求事实。只想结束当前前台 Gateway 和网页时也可以在启动窗口按 `Ctrl+C`。
...
4. 确认客户端全部切换后，再删除旧密钥；删除后使用它的请求会立即失败。
...
- 页面能打开但模型不可用：依次检查资源池状态、上游 API Key 探测与模型绑定、套餐版本路由、活动订阅和 API 密钥的精确模型/资源池授权。
