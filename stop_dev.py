"""Stop every LLM2API development process without deleting its data."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="停止全部 LLM2API 本地开发进程并保留数据")
    parser.parse_args()
    ...
    if command is None:
        ...
        return 2
    print(
        "正在停止 LLM2API 的 Gateway、管理网页、启动器、PostgreSQL 和 Valkey。"
        "命名数据卷会保留。",
        flush=True,
    )
