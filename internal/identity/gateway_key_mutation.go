...
func newGatewayKeyMutation(request MutationRequest, userID uuid.UUID, name string, routes []GatewayKeyRoute, expiresAt *time.Time, replacesKeyID *uuid.UUID) (GatewayKeyMutation, error) {
	...
	}{UserID: userID, Name: name, Routes: routes, ExpiresAt: expiry, ReplacesKeyID: replacesKeyID}
